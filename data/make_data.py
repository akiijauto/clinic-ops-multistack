...
PRICE_CATALOG = {
    "診察料": [
        ("初診料", 1100, True), ("再診料", 550, True), ("時間外診察料", 3300, True),
        ("往診料", 5500, True), ("健康診断料", 2200, True), ("セカンドオピニオン料", 3300, True),
        ("電話再診料", 550, True), ("死亡診断料", 3300, True), ("紹介状作成料", 2200, True),
        ("入院管理料(1日)", 1650, True), ("往診交通費", None, True), ("休日加算", 1100, True),
        ("深夜加算", 2200, True), ("往診時間延長料", 2750, True), ("診断書発行料", 1650, True),
    ],
    "検査料": [
        ("血液一般検査", 3300, True), ("生化学検査(基本項目)", 4400, True), ("生化学検査(追加項目)", None, True),
        ("尿検査", 1650, True), ("糞便検査", 1100, True), ("レントゲン検査(1部位)", 3300, True),
        ("レントゲン検査(追加部位)", 1650, True), ("超音波検査", 4400, True), ("心電図検査", 2750, True),
        ("血液凝固検査", 3850, True), ("アレルギー検査", 6600, True), ("ホルモン検査", 5500, True),
        ("感染症迅速検査", 3300, True), ("細胞診検査", 2750, True), ("血液型検査", 2200, True),
    ],
    "処置料": [
        ("点滴処置(皮下)", 1650, True), ("点滴処置(静脈内)", 2750, True), ("注射(筋肉内)", 880, True),
        ("注射(皮下)", 770, True), ("創傷処置", 1650, True), ("耳道洗浄", 1100, True),
        ("爪切り", 550, True), ("肛門腺絞り", 550, True), ("浣腸処置", 1650, True),
        ("包帯交換", 880, True), ("導尿処置", 2200, True), ("酸素吸入(30分)", None, True),
        ("ネブライザー処置", 1650, True), ("止血処置", None, True), ("胃洗浄", 3300, True),
    ],
    "手術料": [
        ("避妊手術(犬・小型)", 33000, True), ("避妊手術(犬・大型)", 44000, True), ("避妊手術(猫)", 27500, True),
        ("去勢手術(犬)", 22000, True), ("去勢手術(猫)", 16500, True), ("腫瘍摘出術(小)", 27500, True),
        ("腫瘍摘出術(大)", 55000, True), ("歯石除去(全身麻酔下)", 22000, True), ("抜歯処置", 5500, True),
        ("異物除去手術", 66000, True), ("骨折整復術", None, True), ("帝王切開術", 55000, True),
        ("眼瞼形成術", 33000, True), ("会陰ヘルニア整復術", 77000, True), ("麻酔管理料", 5500, True),
    ],
    "予防": [
        ("混合ワクチン(5種)", 6600, True), ("混合ワクチン(8種)", 8800, True), ("狂犬病予防接種", 3500, True),
        ("フィラリア予防薬(1ヶ月分・小型)", 1650, True), ("フィラリア予防薬(1ヶ月分・大型)", 2750, True),
        ("フィラリア抗原検査", 2200, True), ("ノミ・マダニ予防薬(1本)", 2200, True),
        ("駆虫薬(内部寄生虫)", 1650, True), ("猫エイズ・白血病検査", 3850, True), ("マイクロチップ挿入", 5500, True),
        ("狂犬病登録手数料", None, True), ("ワクチン抗体価検査", 5500, True), ("予防相談料", 0, True),
        ("フィラリア予防薬(1ヶ月分・中型)", 2200, True), ("ノミ・マダニ予防薬(お試し1回分)", 1100, True),
    ],
    "入院料": [
        ("入院基本料(1日・小型)", 3300, True), ("入院基本料(1日・大型)", 4400, True), ("集中管理入院料(1日)", 6600, True),
        ("酸素室使用料(1日)", 3300, True), ("入院中投薬料(1日)", 1100, True), ("入院中給餌料(1日)", 550, True),
        ("面会立会料", 0, True), ("入院中点滴管理料(1日)", 1650, True), ("退院時サマリー作成料", 1100, True),
        ("入院中モニタリング料(1日)", 2200, True), ("隔離室使用料(1日)", 4400, True), ("入院中検温料(1回)", 0, True),
        ("入院延長管理料(1日)", 3300, True), ("入院中包帯交換料", 880, True), ("入院準備料", 1100, True),
    ],
    "物品販売": [
        ("療法食(缶・1個)", 440, True), ("療法食(ドライ・1袋)", 3300, True), ("エリザベスカラー", 1650, True),
        ("サプリメント(関節用)", 3850, True), ("サプリメント(皮膚用)", 3300, True), ("歯みがきガム(1箱)", 880, True),
        ("シャンプー(薬用)", 2200, True), ("爪切り用具", 1650, True), ("ペット用消臭剤", 1320, True),
        ("キャリーバッグ", 4400, True), ("介護用リハビリ用品", None, True), ("包帯・ガーゼセット", 660, True),
        ("療法食(猫用・缶)", 440, True), ("サプリメント(整腸用)", 2750, True), ("防水シート(1パック)", 990, True),
    ],
    "その他": [
        ("診断書再発行料", 1100, True), ("カルテ開示手数料", 2200, False), ("ペット霊園紹介料", 0, False),
        ("火葬手配代行料", None, False), ("郵送料", 500, False), ("キャンセル料", 1100, False),
        ("駐車場利用料", 0, False), ("会員登録料", 1100, False), ("ペットシーツ(1枚)", 33, True),
        ("待合室コピー代", 11, True), ("ペット保険相談料", 0, False), ("預り金領収書再発行", 0, False),
        ("院内資料コピー代", 22, True), ("往診用消耗品費", 880, True), ("その他雑費", 0, False),
    ],
}
