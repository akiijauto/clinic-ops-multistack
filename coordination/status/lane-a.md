...
## 共通ナビ・タイトル対応（2026-09-06）

指揮役の実測「34画面が契約どおりでない」を受けて、`layouts/base.html` 1箇所に
共通ナビを追加した（個別画面には書かない）。

### やったこと

1. **`web/templates/layouts/base.html`**
   - `<title>` を `{{block "title" .}} — 動物病院 窓口業務システム` の形にした
     （各画面の `title` ブロックは元から画面名だけだったので、末尾を足すだけで済んだ）。
   - `<body>` 直下に共通ナビ（9本・契約の順序どおり: 本日の患者/検索/予約/入院/DM/
     売上集計/スタッフ/設定/このシステムについて）を追加。**全画面がこの1レイアウトを
     経由する**（`internal/view/view.go` が `layout` を唯一のエントリにしている）ので、
     個別画面を触らずに済んだ。
   - ナビは最初 `<nav><ul><li>` で書いたが、`spec/ui.css` の `nav a { margin-right }` は
     `<nav>` 直下の `<a>` を前提にしていて、`<ul><li>` で包むと既定の縦積み箇条書きに
     なり Laravel/FastAPI と見た目が割れた（`tests/shots.py` で実測）。
     `<nav><a>...</a></nav>` の並びに直して、水平のバーに揃えた。

2. **`web/templates/pages/top.html`**
   - `title` ブロックを `窓口業務 トップ` → `トップ` に変更（末尾はレイアウト側が足す）。
   - トップ独自の`<nav>`（26画面ぶんのリンク、共通ナビと重複していた）を撤去し、
     共通ナビとは別の `<div data-testid="top-screen-links">` に移した
     （動物・伝票・予約IDに依存する個別画面への入口。検算8の深さ2クロール用に
      引き続き必要なので消さず、置き場所だけ変えた）。
   - `<h1>` は指揮役の修正のまま触っていない。
...
$ python tests/run.py http://127.0.0.1:8401
全 25 件 通過
（「見た目 トップの見出しと共通ナビが契約どおり」含む）

$ python tests/run.py http://127.0.0.1:8401 --only inventory
全 11 件 通過 — 33 画面で見出しとナビを確認

$ go build ./... && go test ./...
全ビルド・テスト成功（apperr/billing/clinical/server/view/web すべて ok）
```

`tests/shots.py / /about /today` で実際にブラウザ描画も確認: Go は
`title=画面名 — 動物病院 窓口業務システム` `h1=画面名` `nav=9本`（先頭9件が契約どおりの
順序）で、FastAPI（既に全緑）と同じ見た目になった。今回サーバは同じスクラッチパッドの
