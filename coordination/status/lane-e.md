...
## 2026-09-06 トップ画面と共通ナビを契約どおりに揃えた（指揮役指示への対応）

原因と対処は `coordination/qa/lane-e.md`「トップ画面と共通ナビを契約どおりに揃える」節を参照。
要点: 共通ナビが `<header>` に入っていた／`render.ts`側の`<nav>`にはsettings専用4本しか
入っていなかった（判定器は`<nav>`しか見ないため「navなし」「8本欠け」に見えていた）。
`<h1>`は`area1/html.ts`の`page()`が自動生成するよう変更し、書き忘れを構造的に無くした。
`<title>`は実装名を除いた`画面名 — 動物病院 窓口業務システム`に統一。
`/folded`（key無し）は契約に無いため404のまま維持（判断理由は同ファイルに記載）。

`python tests/run.py http://127.0.0.1:8405` の出力:
...
── inventory ──
  ...
  OK  検算4 カルテの画面と印刷で同じ値が出る  — 16 組を比べて差なし
  OK  検算5 基準の外にある値は判定欄と色の両方に出る  — 50 項目の判定が一致
  OK  検算6 予約が担当・処置室のどちらでも重ならない  — 65 件で重なり0
  OK  検算7 入院の記録行に実施者が必ず入っている  — 108 件中 実施者なし 0 件
  OK  検算9 削除済みは一覧から消えるが件数には残る  — 一覧から消えても集計に残る
  ...
  OK  書き込み 作って・確かめて・取り消せる（GETだけでは分からない）  — 作成→取消の往復ができた（id=66）
...
`npm run typecheck` エラー0／`BASE_URL=http://127.0.0.1:8405 npm test` 44/44。
`tests/shots.py`（実ブラウザ描画）で `/` `/today` `/about` `/settings` `/sales` `/reservations`
`/staff` `/dm` `/ward` を確認し、h1はLaravel/Go/Rails/FastAPIといずれも一致。
`<nav>` `<div>` 以外は書き換えていません。待機します。

## 2026-09-06 続報: ナビの余分3本・トップ本文7本リンクを対処

原因と対処は `coordination/qa/lane-e.md`「続報: ナビの中身が画面ごとに違っていた／
トップ本文がナビの複製だった」節を参照。要点: `render.ts`のsettings専用4本が
共通`<nav>`に混ざっていた（`<div>`へ分離・settings系画面限定に）。トップ本文は
`/folded` `/about`への重複リンクを外し`/today`1本のみに。ナビの区切り記号（｜）も
全箇所で削除（`ui.css`の`nav a{margin-right}`に任せる）。
...
── inventory ──
  ...
  OK  検算4 カルテの画面と印刷で同じ値が出る  — 16 組を比べて差なし
  OK  検算5 基準の外にある値は判定欄と色の両方に出る  — 50 項目の判定が一致
  OK  検算6 予約が担当・処置室のどちらでも重ならない  — 72 件で重なり0
  OK  検算7 入院の記録行に実施者が必ず入っている  — 108 件中 実施者なし 0 件
  OK  検算9 削除済みは一覧から消えるが件数には残る  — 一覧から消えても集計に残る
  OK  検算8 画面から辿れるリンクが全部生きている  — 60 画面を辿って切れなし
  OK  在庫 契約の画面ルートが全部ある（未実装はクローラーに見えない）  — 40/42 件ある（2 件は確かめられない: /papers/{paper_id}, /papers/{paper_id}/remove）
  ...
  OK  見た目 指示したクラスが実際に画面に付いている  — 37 画面で 3 クラスすべてを確認
  OK  契約 押しても何も起きないボタンにしない（灰色3つが理由へ繋がる）  — 3 個: karte-temp-save, reception-complete-delete, reception-complete-delete-all
  OK  書き込み 作って・確かめて・取り消せる（GETだけでは分からない）  — 作成→取消の往復ができた（id=73）
  OK  データ 実装が読んでいる中身が data/ と一致する（数字だけ見ても分からない）  — 513 項目が data/ と一致
  OK  見た目 トップの見出しと共通ナビが契約どおり（5実装で同じに見えるか）  — 35 画面で見出しとナビを確認
...
`npm run typecheck` エラー0／`BASE_URL=http://127.0.0.1:8405 npm test` 44/44。
`tests/shots.py`で`/` `/about` `/settings` `/folded`を確認、nav本数が5実装とも
10本（`/settings`含む）で揃った。待機します。

## 2026-09-06 続報2: 画面名（h1/title）をナビの表示と揃えた

`/dm`（DM管理→DM）・`/settings`（設定（病院設定）→設定）・`/ward`（入院（一覧）→入院）の
`title`引数を修正。`page()`が`<h1>`/`<title>`を自動生成する形にしてあったため呼び出し側の
文字列を直すだけで済んだ。ついでに`/reservations`のPOST失敗時（同じ`screen-reservations`）の
`予約（新規）`も`予約`に統一。詳細は`coordination/qa/lane-e.md`参照。
...
── inventory ──
  ...
  OK  検算4 カルテの画面と印刷で同じ値が出る  — 16 組を比べて差なし
  OK  検算5 基準の外にある値は判定欄と色の両方に出る  — 50 項目の判定が一致
  OK  検算6 予約が担当・処置室のどちらでも重ならない  — 82 件で重なり0
  OK  検算7 入院の記録行に実施者が必ず入っている  — 108 件中 実施者なし 0 件
  OK  検算9 削除済みは一覧から消えるが件数には残る  — 一覧から消えても集計に残る
  OK  検算8 画面から辿れるリンクが全部生きている  — 60 画面を辿って切れなし
  OK  在庫 契約の画面ルートが全部ある（未実装はクローラーに見えない）  — 40/42 件ある（2 件は確かめられない: /papers/{paper_id}, /papers/{paper_id}/remove）
  ...
  OK  見た目 指示したクラスが実際に画面に付いている  — 37 画面で 3 クラスすべてを確認
  OK  契約 押しても何も起きないボタンにしない（灰色3つが理由へ繋がる）  — 3 個: karte-temp-save, reception-complete-delete, reception-complete-delete-all
  OK  書き込み 作って・確かめて・取り消せる（GETだけでは分からない）  — 作成→取消の往復ができた（id=83）
  OK  データ 実装が読んでいる中身が data/ と一致する（数字だけ見ても分からない）  — 513 項目が data/ と一致
  OK  見た目 トップの見出しと共通ナビが契約どおり（5実装で同じに見えるか）  — 35 画面で見出しとナビを確認
  OK  見た目 トップの本文が契約どおり（ナビの複製を並べない）  — 説明あり・本文のリンク 5 本
  OK  契約 本日の患者に受付区分のタブがある  — 6 区分すべてにタブがある — ★ ただし本日の受付が0件のため、絞り込みが効いているかは確かめられていない（データがanchor_dateに固定されているため）
...
`npm run typecheck`エラー0／`BASE_URL=http://127.0.0.1:8405 npm test` 44/44。
`tests/shots.py /ward /settings /dm` → 3画面とも「揃っている」。担当分は完了。待機します。

## 2026-09-06 続報3: ナビに無い画面の名前を契約(summary)に揃えた

原因と対処は`coordination/qa/lane-e.md`「続報3: ナビに無い画面（患者名を見出しに混ぜていた）」
節を参照。要点: カルテ/検査/会計/顧客/来院履歴/書類/投薬/予防/削除確認の各画面が
`<h1>`/`<title>`に患者名・種別名を混ぜていた（患者ごとに画面名が変わる）ため、
`page()`へ渡す`title`を契約の`summary`に統一。患者名は本文冒頭の識別行に出す形に
（無かった顧客・来院履歴の2画面には新規追加）。1件だけ（`/karte/copy_prev`）契約の
summaryが「前回コピー」で他と違うため、`renderKarteScreen()`にtitleの省略可能引数を足した。
...
── inventory ──
  ...
  OK  検算4 カルテの画面と印刷で同じ値が出る  — 16 組を比べて差なし
  OK  検算5 基準の外にある値は判定欄と色の両方に出る  — 50 項目の判定が一致
  OK  検算6 予約が担当・処置室のどちらでも重ならない  — 88 件で重なり0
  OK  検算7 入院の記録行に実施者が必ず入っている  — 108 件中 実施者なし 0 件
  OK  検算9 削除済みは一覧から消えるが件数には残る  — 一覧から消えても集計に残る
  OK  検算8 画面から辿れるリンクが全部生きている  — 60 画面を辿って切れなし
  OK  在庫 契約の画面ルートが全部ある（未実装はクローラーに見えない）  — 40/42 件ある（2 件は確かめられない: /papers/{paper_id}, /papers/{paper_id}/remove）
  ...
  OK  見た目 指示したクラスが実際に画面に付いている  — 37 画面で 3 クラスすべてを確認
  OK  契約 押しても何も起きないボタンにしない（灰色3つが理由へ繋がる）  — 3 個: karte-temp-save, reception-complete-delete, reception-complete-delete-all
  OK  書き込み 作って・確かめて・取り消せる（GETだけでは分からない）  — 作成→取消の往復ができた（id=89）
  OK  データ 実装が読んでいる中身が data/ と一致する（数字だけ見ても分からない）  — 513 項目が data/ と一致
  OK  見た目 トップの見出しと共通ナビが契約どおり（5実装で同じに見えるか）  — 35 画面で見出しとナビを確認
  OK  見た目 トップの本文が契約どおり（ナビの複製を並べない）  — 説明あり・本文のリンク 5 本
  OK  契約 本日の患者に受付区分のタブがある  — 6 区分すべてにタブがある — ★ ただし本日の受付が0件のため、絞り込みが効いているかは確かめられていない（データがanchor_dateに固定されているため）
...
`npm run typecheck`エラー0／`BASE_URL=http://127.0.0.1:8405 npm test` 44/44。
`tests/shots.py`（全画面）で`/animals/{karte_no}/karte` `/exam` `/accounting`の
h1がRails/FastAPIと一致（患者名を含まない）ことを確認。担当分は完了。待機します。

## 2026-09-06 続報4: `/today`の既定表示が全25件出ていた件（データ再投入で顕在化）

原因は`scripts/seed.ts`が`reception.kind`列を上書きしておらず、全25件が列のDEFAULT値
`'first_visit'`のまま入っていたこと（詳細は`coordination/qa/lane-e.md`参照）。
`medical_purpose`（区分の表示名）を`data/masters.json`の`reception_kinds`と逆引きして
`kind`を求めるよう`seed.ts`を修正し、`next dev`を止めて`npm run seed`→再起動、`/today`の
既定表示が1行（初診）になることを確認した。
