...
## 2026-09-06 — 開発DBを `data/seed.json` から入れ直し（指揮役の実測指摘への対応）

owner id=18 の姓が古いデータのままだった件（指揮役実測）に対応。`bundle exec rails db:seed`
で入れ直し、95項目のサンプル突合で全一致・売上総合計も変化なし（5,185,704円）を確認した。
詳細は `coordination/qa/lane-b.md` Q21。

### 完了の自己点検

```
$ python tests/run.py http://127.0.0.1:8414 --only inventory
（10件。新規「書き込み 作って・確かめて・取り消せる」「データ 実装が読んでいる中身がdata/と一致する」含む）
全 10 件 通過
```
...
**別件（レビュー役指摘・未対応）**: カルテ画面に保存・新規診察・印刷への導線が無い件、
ナビに`/dm`・`/sales`へのリンクが無い件は、指揮役の指示どおり「データの入れ直しを先に」
対応したため、まだ着手していない。次の指示待ち。

---

## 2026-09-06 — 共通ナビ・題名の統一、残っていた2件を対応

指揮役の指示に沿って以下を実施（`stacks/rails/` のみ変更）。

1. **共通ナビ**: `app/views/layouts/application.html.erb` に `<nav>` を1本追加。
   契約どおりの順序で9本（トップを含め10本）。全画面に効く。
2. **`<title>`**: `画面名 — 動物病院 窓口業務システム` の形に統一（レイアウト側で
   `content_for(:title)` に病院名を自動で足す形にした）。既存ビューのほとんどは
   すでに `content_for :title` を持っていたため、抜けていた3画面
   （削除確認・折りたたみ表示・ToDo）にだけ追加。
3. **`/today` の `<h1>`**: 「本日の患者（受付一覧）」→「本日の患者」に統一。
4. **カルテ画面の導線（以前からの指摘）**: 実際には保存・新規診察・印刷のリンクは
   既に実装済みだった（`karte/_visits.html.erb`）。今回わかった不足は
   **`<h1>` が無かったこと**で、これを追加（`カルテ — {患者名}（{カルテNo}）`、
   Laravel と同じ形）。
5. **`/folded` の例外画面（以前からの指摘）**: 単体アクセスで Routing Error が出ていた。
   `FoldedController#show` を key 無しでも受けるルートを追加し、全項目の一覧を返す形にした
   （406/404を止めて200に）。

### 副作用として見つけて直したもの

`config/environments/development.rb` の `annotate_rendered_view_with_filenames = true`
（Rails既定でON）が、HTMLコメントに `app/views/folded/show.html.erb` のような文字列を
埋め込んでおり、共通クローラーの `/folded/([語])` 抽出がこれを誤って拾って
`/folded/show.html.erb` を叩き404、という副作用が出た（`/folded` 単体を200にした結果、
このコメントが最初に見える候補になったため顕在化）。実装の比較企画としてもファイルパスが
画面に出るのは望ましくないため、この設定をoffにした（要サーバ再起動、実施済み）。
...
`python tests/shots.py /` と `python tests/shots.py /animals/10003/karte` で
実ブラウザ描画も確認。トップは Laravel/Next.js と title・h1・nav（10本）が一致。
カルテは nav・h1（`カルテ — モモ（10003）`）とも表示された
（`tests/shots/animals_10003_karte` 配下の古いスクリーンショットは今回の修正前のもので、
再撮影して確認済み）。
...
## 2026-09-06 — トップの本文をナビと重複しない形に絞る

指揮役の指摘（オーナーが5実装を並べ、トップ本文にナビの複製が14本あると発覚）に対応。

1. `app/views/top/index.html.erb` — ナビと同じ行き先を並べた `<ul>` 7本を削除。
   本文は「学習・研究目的」「データは架空」「スコープ外機能の案内（折りたたみ表示・
   このシステムについて）」「本日の患者への導線1本」だけに絞った（Laravelを手本にした）。
2. `app/views/layouts/application.html.erb` — 上記だけでは本文リンクが8本のままだった。
   原因はRails既定のfavicon（`icon.png`×2・`icon.svg`）と中身の無い
   `stylesheet_link_tag :app` が `<head>` に残っていたこと。共通クローラーは
   `<nav>` の外の href をすべて数えるため、これも本文リンクに数えられていた。
   他4実装にはfaviconが無いため削除し、`<link>` は `/ui.css` だけにした。
...
`python tests/shots.py /` で実ブラウザ確認。Laravel/Go/Next.jsとnav本数（10）が揃い、
本文もLaravelと同じ4本構成（folded・about・todayへの説明リンク＋導線）になった。

指揮役から指摘のあった「`/folded/{key}` が404」「カルテのh1が空」の2件は、
直接アクセスで再確認したところ既に解消していた（前回報告時点はサーバ再起動の
