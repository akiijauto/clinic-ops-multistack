...
## 完了の自己点検（7回目、トップ画面の見た目差分2件対応）

指揮役から2件のNG報告（ナビに「トップ」が無い／トップの本文が「本日の患者」の中身
そのまま）を受けて対応。

- `app/templates/base.html`: 共通ナビの先頭に `<a href="/">トップ</a>` を追加（10本、
  契約の並び順どおり）。
- `app/templates/front/today.html`: `{% if screen_key == "top" %}` で本文を完全に
  分岐。トップは h1・説明（学習研究目的／データは架空／5スタック実装比較）・
  `visit_count.today`（screens.md 102行目でトップにも要求されている）・`/today` への
  導線1本のみ。ナビと同じ行き先は本文に置いていない。`/today` 側の中身・目印
  （`data-testid`/`data-check`）は変更していない。
- サーバーは `--reload` なしで起動していたため、テンプレート変更後に一度プロセスを
  止めて `uvicorn app.main:app --host 127.0.0.1 --port 8415` で入れ替え起動した
  （孤児プロセスが出ていないことを `netstat`/`Get-Process` で確認）。
...
`tests/shots.py /` でも確認: 5実装とも h1=「動物病院 窓口業務システム」・nav=10本で一致。
唯一残る差は `<title>` の書式（Go/Rails/Next.jsは「トップ — 動物病院 窓口業務システム」、
