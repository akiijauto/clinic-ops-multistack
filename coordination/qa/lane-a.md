...
## トップ本文の26本リンクをどうしたか（2026-09-06）

指揮役から「画面に出さない形にする／一覧そのものを別画面へ移す、どちらかを選び理由を書け」
と依頼された件。**どちらも選ばず、削除した。** 理由は以下。

### 「画面に出さない」は検査を通らない

新しい `_top_body` チェック（`tests/inventory.py`）は、共通ナビ（先頭の `<nav>...</nav>`）を
正規表現で取り除いた**残りのHTML文字列**から `href="..."` を素朴に数える。

    body = re.sub(r"<nav[^>]*>.*?</nav>", "", html, flags=re.S)
    links = re.findall(r'href="([^"]+)"', body)

これは**見えるかどうかを見ていない**。`display:none` で隠しても、HTMLソースに
`href` が残っていれば検査には引っかかる。CSSで隠す案は「人には見えない」は満たすが
「本文のリンクが6本以内」は満たせない。

### 「別画面へ移す」も、素直な移し先が無い

`spec/screens.md` 26番「このシステムについて」は「できること: 読むだけ。
『折りたたみ表示』『機能設定』への導線を持つ」と**すでに2本と決まっている**。
ここへ26本を足すと、about側の契約（導線は2本）を壊す。新しい画面を1つ増やす手も
あるが、契約に無い画面を増やす判断は指揮役の領分だと考え、やらなかった。

### 実際にやったこと：削除して、検算8を実測で確認

`spec/screens.md`「トップ画面の本文」の決めごと（h1・説明・本日の患者への導線1本、
これ以外は本文に置かない）を素直に読むと、そもそも26本の一覧を**どこかに残す**
という前提が要らない。検算8（死んだリンク）は「トップから直接」ではなく
「`/` から辿れる全体」を見ているので、共通ナビ（10本）→本日の患者・検索・設定などの
各画面が自然に持つ導線（患者行→カルテ/検査/会計、設定→機能設定/取込/マスタ 等）で
十分に深さが出るはずだと考え、削除して実測した。

```
$ python tests/run.py http://127.0.0.1:8401 --only crawl
検算8 画面から辿れるリンクが全部生きている — 60 画面を辿って切れなし
```

（上限60件のキャップに達しており、10件という閾値も余裕で超えている。削除前も
「60画面」だったので、実質的な低下は無かった。）

副作用として、削除前は40/42件あった「契約の画面ルートが全部ある」在庫チェックが
39/42件（`/folded/{key}` が実例経由で確かめられなくなった）になったが、この検査は
